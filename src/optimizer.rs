...
impl Optimizer {
    pub fn new() -> Self {
        Self {
            wasm_opt_path: which::which("wasm-opt").ok().map(|p| p.to_string_lossy().to_string()),
        }
    }
    
    pub async fn optimize_file(&self, input: &Path, output: &Path, level: u8) -> Result<()> {
        if let Some(wasm_opt) = &self.wasm_opt_path {
            self.optimize_with_wasm_opt(wasm_opt, input, output, level).await
        } else {
            self.optimize_manual(input, output, level).await
        }
    }
    
    async fn optimize_with_wasm_opt(&self, wasm_opt: &str, input: &Path, output: &Path, level: u8) -> Result<()> {
    ...
    fn write_custom_section(&self, wasm: &mut Vec<u8>, reader: &wasmparser::CustomSectionReader) -> Result<()> {
